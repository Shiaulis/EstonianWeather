...
final class SWXMLParser {
    ...
    init(context: NSManagedObjectContext, languageCode: String, logger: Logger = PrintLogger()) {
...
extension SWXMLParser {
    ...
    private func forecasts(from indexer: XMLIndexer) throws -> [Forecast] {
        try indexer[.forecasts][.forecast].all.map { try forecast(from: $0) }
    }
    ...
    private func forecast(from indexer: XMLIndexer) throws -> Forecast {
        ...
        do {
            mappedForecast = try existingForecast(for: forecastDate)
        }
        catch NSManagedObjectContext.Error.noObjectFoundInDatabase {
            mappedForecast = try .create(in: self.context)
        }
        ...
        mappedForecast.night = try dayPartForecast(from: indexer[.night])
        mappedForecast.night?.type = NSLocalizedString("Night", comment: "forecast part type")

        mappedForecast.day = try dayPartForecast(from: indexer[.day])
    ...
    private func dayPartForecast(from indexer: XMLIndexer) throws -> DayPartForecast {
        let dayPartForecast: DayPartForecast = try .create(in: self.context)
        dayPartForecast.type = NSLocalizedString("Night", comment: "forecast part type")
        dayPartForecast.phenomenon = phenomenon(from: indexer[.phenomenon])
        ...
        let placesArray: [Place] = indexer[.place].all.compactMap { place(from: $0) }
        dayPartForecast.places = Set(placesArray)

        let windsArray: [Wind] = indexer[.wind].all.compactMap { wind(from: $0) }
    ...
    private func place(from indexer: XMLIndexer) -> Place? {
        let place: Place
        do {
            place = try .create(in: self.context)
        }
        catch {
        ...
        place.phenomenon = phenomenon(from: indexer[.phenomenon])
    ...
    private func wind(from indexer: XMLIndexer) -> Wind? {
        let wind: Wind
        do {
            wind = try .create(in: self.context)
        }
        catch {
            let error = Error.elementNotFound(identifier: Element.place.rawValue)
            self.logger.log(message: "This case is not expected", error: error, module: .dataParser)
    ...
    private func phenomenon(from indexer: XMLIndexer) -> Phenomenon? {
        ...
        do {
            return try existingPhenomenon(for: phenomenonString)
        }
        catch NSManagedObjectContext.Error.noObjectFoundInDatabase {
            do {
                let phenomenonToAdd: Phenomenon = try .create(in: self.context)
...
extension SWXMLParser {
    ...
    private func existingForecast(for date: Date) throws -> Forecast {
        ...
        return try self.context.fetch(request: request)
    }
